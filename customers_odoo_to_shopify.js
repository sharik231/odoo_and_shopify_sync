...
function getLastSyncTimes() {
  if (!fs.existsSync(".lastsync_customers.json")) return { odoo: null };
...
export async function syncCustomersOdooToShopify() {
  ...
  if (USE_LAST_SYNC && lastOdooSync) domain.push(["write_date", ">", lastOdooSync]);
  ...
  console.log(`📦 Found ${customers.length} customers in Odoo`);
...
(async () => {
  console.log("⏳ Starting Odoo → Shopify Customer Sync...");
  await syncCustomersOdooToShopify();
  console.log("✅ Finished Odoo → Shopify Sync");
})();
